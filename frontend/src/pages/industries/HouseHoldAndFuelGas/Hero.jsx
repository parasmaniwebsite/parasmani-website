...
import blueprintImg from "../../../assets/industries/household/heroImg1.png"; 
import realisticImg from "../../../assets/industries/household/heroImg2.png"; 
...
Household & Fuel Gas
          ...
          >
Parasmani copper tubes for PNG and LPG distribution in residential buildings, hotels, commercial kitchens, and institutional gas networks. Non-combustible, non-permeable, built to last a lifetime.
          </motion.p>
...
Download Gas Piping Brochure
        ...
        {/* Right Interactive Image Side.
            ...
            Both source PNGs are 3840x2160 with the building pushed hard to the
            right of the canvas -- heroImg1 carries 1248px of transparent margin
            on the left against 518px on the right, heroImg2 1287px against
            487px. `object-contain` centres the canvas, not the artwork, so the
            building lands right of centre with no correction at all.

            The negative translate below is that correction, and it has to be a
            percentage rather than a fixed px nudge: the offset scales with the
            container, so any pixel value is only correct at one viewport width.
            Each value is its own image's measured offset (9.51% and 10.42% of
            canvas width) multiplied by 1.35 to cancel `scale-135`, since
            `translate` composes outside `scale` and is not scaled by it. Above
            `md` the scale drops to 1 and the original -3.5rem nudge applies.

            Re-measure these if either PNG is ever re-exported. Trimming the
            transparent margins from the sources would remove the need for them
            entirely, but changes the aspect ratio and so the rendered size. */}
        <div className="md:col-span-6 lg:col-span-7 relative w-full aspect-[3/2] flex justify-center items-center">
          ...
          <motion.img
            ...
            className="absolute inset-0 w-full h-full object-contain pointer-events-none will-change-transform -translate-x-[12.84%] scale-135 mt-10 md:mt-0 md:scale-100 md:-translate-x-14"
          />
          ...
          <motion.img
            ...
            className="absolute inset-0 w-full h-full object-contain pointer-events-none will-change-transform -translate-x-[14.07%] scale-135 mt-10 md:mt-0 md:scale-100 md:-translate-x-14"
          />
        </div>
